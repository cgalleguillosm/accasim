...
The dispatcher instance is composed by  implementations of the abstract *SchedulerBase* and *AllocatorBase* classes. In this illustrative 
instantiation of the *Simulator* class, *FirstInFirstOut* implements *SchedulerBase* using FIFO, whereas *FirstFit* implements *AllocatorBase* 
using  First Fit allocation policy, and both  *FirstInFirstOut* and  *FirstFit* classes are available in the library for importing, as done in lines 2-3 of the example. 
AccaSim can be customized in its dispatching method by implementing the abstract *SchedulerBase* and *AllocatorBase* classes as desired. 